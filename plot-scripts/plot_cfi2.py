...
def plot_figs(dfs):
    ...
    for i in range(0, m):
        for j in range(0, n):
            ...
            for bar, color, pattern, label in zip(bars, colors, patterns, labels):
    ...
    fig.tight_layout()
    plt.savefig('./avg-cfi.pdf')

    with open("avg-cfi.csv", 'w') as file:
    ...
    with open("max-cfi.csv", 'w') as file:
